...
Handles KeePass decryption (in RAM), HID injection, Rate Limiting, and Sync mode.
"""
import os
import sys
import time
...
KEY_CODES = {
    ...
    '\n': 40, 'ENTER': 40, 'ESC': 41, 'BACKSPACE': 42, 'TAB': 43, 'SPACE': 44, '-': 45, '=': 46,
...
def scrub_ram(obj: Any):
    ...
    # does not scrub the original. Rely on deleting references and GC for strings.
...
class SecurityMonitor:
    ...
    def register_request(self, target_id: int):
        now = time.time()
        self.recent_requests = [t for t in self.recent_requests if now - t < HONEYPOT_WINDOW]
...
def send_keystroke(keycode: int, shift: bool = False):
    ...
    try:
        ...
        pass # Handle write errors silently to avoid crash loops
...
def generate_sync_payload(kp: Any) -> str:
    ...
    for idx, entry in enumerate(kp.entries):
        if entry.title == "Meta-Info": continue
        data.append({
            'id': idx + 1,
    ...
    try:
        with open(SYNC_SECRET_PATH, 'rb') as f:
            secret = f.read().strip()
            mac = hmac.new(secret, b64_payload.encode('utf-8'), hashlib.sha256).hexdigest()
            signature = f"{mac}:"
    except FileNotFoundError:
