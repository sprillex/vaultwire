...
def draw_main_menu(stdscr: curses.window, data_file: str) -> None:
    ...
    curses.init_pair(
        1,
        curses.COLOR_BLACK,
        curses.COLOR_GREEN)  # Active cursor bar
    ...
    while True:
        ...
        stdscr.addstr(
            4, 4, "• Browse to find the service ID you want to log into.")
        stdscr.addstr(
            5,
            4,
            "• Manually type the ID into the Pi's Bluetooth keyboard "
            "(Development Phase) to trigger injection.")
        stdscr.addstr(
            6,
            4,
            "• Press [S] to enter Sync Mode and import current service "
            "metadata from the Pi.")
        ...
        if not vault_data:
            stdscr.addstr(
                data_start_y,
                4,
                "[!] No local service index found. Press [S] to sync "
                "with your Pi.",
                curses.A_BLINK)
        else:
        ...
        if key == curses.KEY_UP and current_row > 0:
            ...
            # Temporarily exit main UI loop to handle standard string
            # absorption
            ...
            if current_row < len(vault_data):
                ...
                status_message = (f"Selected ID is '{selected_id}'. Type this "
                                  "on the Pi keyboard to inject!")
